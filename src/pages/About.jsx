...
const About = ({ isAuthenticated }) => {
  const [aboutContent, setAboutContent] = useState({
    title: 'About Claymores Rugby',
    content: `The Central Florida Claymores RFC began life in 2018 as the newest member to the Florida Rugby Union. The task at hand, to grow a competitive team built by players for players while simultaneously growing the historic sport of rugby.\n\n
    ...
    coachTitle: 'About Our Coach',
    coachContent: `Jamie Moncur started his playing career with 10 years of school boy rugby in Edinburgh Scotland at the well established programs of Stewart’s Melville and Trinity Academy.\n\n
    ...
    <div className="about-container">
      <div className="about-content">
        ...
        {isAuthenticated ? (
          ...
          <p>{aboutContent.content.split('\n').map((str, index) => <span key={index}>{str}<br /></span>)}</p>
        )}

        {isAuthenticated ? (
          <input
            type="text"
            value={aboutContent.coachTitle}
            onChange={(e) => handleContentChange('coachTitle', e.target.value)}
            className="editable-input"
          />
        ) : (
          <h1>{aboutContent.coachTitle}</h1>
        )}
        ...
        {isAuthenticated ? (
          <textarea
            value={aboutContent.coachContent}
            onChange={(e) => handleContentChange('coachContent', e.target.value)}
            className="editable-textarea"
          />
        ) : (
          <p>{aboutContent.coachContent.split('\n').map((str, index) => <span key={index}>{str}<br /></span>)}</p>
        )}
      </div>
